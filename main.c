 ...
 * Author: josej
 *
 * Created on May 18, 2022, 10:00 AM
 */
...
void __interrupt() isr (void){
    ...
    if(PIR1bits.ADIF){                  // Fue interrupción del ADC
        ...
        if (PORTBbits.RB0 == 0){        // Fue RB0 
            if (bandera_sleep ==0){     // Está despierto
                bandera_sleep =1; }     // Mandarlo a dormir
        }
        
        if (PORTBbits.RB1 == 0){        // Fue RB1 
            if (bandera_sleep ==1){     // Está dormido
...
void setup(void){
    ...
    INTCONbits.PEIE = 1;        // Habilitamos interrupción de perifericos
    INTCONbits.GIE = 1;         // Habilitamos interrupciónes globales
    
}

// Leer EEPROM
uint8_t read_EEPROM(uint8_t address){
    EEADR = address;            // Guardamos dirección a leer
...
void write_EEPROM(uint8_t address, uint8_t data){
    EEADR = address;            // Guardamos dirección a escribir
